...
class UtilTest(unittest.TestCase):
    ...
    def test_getErrorBitsTable(self):
        ...
        np.testing.assert_almost_equal(t, xp.array([[0,1,1,2,1,2,2,3],[1,0,2,1,2,1,3,2],[1,2,0,1,2,3,1,2],[2,1,1,0,3,2,2,1],[1,2,2,3,0,1,1,2],[2,1,3,2,1,0,2,1],[2,3,1,2,1,2,0,1],[3,2,2,1,2,1,1,0]]))
    ...
    def test_CayleyTransform(self):
        U = util.CayleyTransform(util.asnumpy(util.getRandomHermitianMatrix(4)))
        np.testing.assert_almost_equal(util.asnumpy(U.dot(U.conj().T)), np.eye(4, dtype=np.complex))
        U = util.CayleyTransform(util.asnumpy(util.getRandomHermitianMatrix(8)))
